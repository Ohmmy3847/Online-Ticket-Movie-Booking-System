...
function updateSeatSummary() {
    const lastprice = parseFloat(document.getElementById("lastpayment").value);
    ...
    const summaryContainer = document.getElementById("seatsummary");
    let budget = lastprice-totalPrice
    let lastseatID = document.getElementById("seatIDS").value
    let lastseatIDArray = lastseatID.split(',').map(Number);
      ...
      <div class="flex flex-row justify-start  md:justify-center bg-black text-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="w-1/2 p-4 md:items-left">
          <div class="space-y-2">
            ...
            <h3 class="text-lg md:text-base mt-4 md:text-left">
    เงินเหลือ <span class="${budget < 0 ? 'text-red-600' : 'text-green-600'}">
        ${budget.toFixed(2)} ฿
    </span>
</h3>
      ...
      ${seatNames.length > 0 ? `
        <div class="mt-6 text-center md:text-left ">
          <form action="/book_editing" method="POST">
             ...
             <input type="hidden" name="lastSeatID" value='${JSON.stringify(lastseatIDArray)}'>
...
function chooseSeat(seatId, seatName, price, element) {
    ...
    console.log(listselected_ticket)
...
async function getseat(showtimeId, start_time, BranchName, TheaterNumber) {
    ...
    let seatsid = document.getElementById("seatIDS").value
    let seatIDsArray = seatsid.split(',').map(Number);
        ...
        <div class="screen md:w-3/5 w-4/5"></div>
            ...
            for (const row in groupedSeats) {
                ...
                groupedSeats[row].forEach((seat, index) => {
                    ...
                    } else if (seat.SeatStatus === "Occupied" && seatIDsArray.includes(Number(seat.SeatID))) {
                        seatClass = "bg-yellow-600";
                        listselected_ticket.push(seat)
